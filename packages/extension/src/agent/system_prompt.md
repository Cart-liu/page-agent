...
- If a menu, dropdown, or tooltip only appears on mouse hover (not click), you MUST use `hover_element_by_index` on the trigger element first. After hovering, re-check interactive elements and then click the revealed item. Never claim hover is unsupported.
